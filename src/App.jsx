  ...
  const scrollToPhase = (phaseName) => {
    ...
    setTimeout(() => {
      ...
      if (element) {
        ...
        element.style.boxShadow = "0 0 0 2px #3B82F6, 0 0 20px rgba(59,130,246,0.5)";
  ...
  return (
    <div className={`flex h-screen overflow-hidden theme-wrapper ${isDarkMode ? 'theme-dark' : 'theme-light'}`}>
          ...
          <div>
            ...
            <nav className="space-y-1">
              {[
                ...
                { id: 'status', label: '회차별 현황', icon: <Kanban size={20} strokeWidth={1.5} /> },
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="shrink-0 border-b pl-4 pr-[140px] lg:pl-6 lg:pr-[140px] py-3 flex flex-wrap lg:flex-nowrap items-center justify-between gap-4 transition-colors" style={{ background: 'var(--bg-header)', borderColor: 'var(--border)', backdropFilter: 'blur(12px)', WebkitAppRegion: 'drag' }}>
          <div className="flex items-center gap-4 min-w-0">
            <div className="flex items-center gap-2 text-[13px] whitespace-nowrap" style={{ color: 'var(--text-dim)' }}>
              ...
              <span className="font-bold" style={{ color: 'var(--text-muted)' }}>{activeMenu === 'timeline' ? '일정 타임라인' : activeMenu === 'status' ? '회차별 현황' : '사업 목록'}</span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide p-4 lg:p-6 transition-colors">
            ...
            <div className="rounded-2xl p-4 lg:p-5 border flex flex-col transition-all relative overflow-hidden" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-light)' }}>
              ...
              <div className="flex-1 flex flex-wrap content-start gap-2 pt-4 border-t z-10" style={{ borderColor: 'var(--border-light)' }}>
                ...
                {donePhases.map(phaseName => (
                  ...
                  >
                    {phaseName} 완료 <ChevronRight size={12}/>
                  </button>
                ))}
              </div>
            </div>
          ...
          <nav className="flex items-center gap-3 mb-6 overflow-x-auto scrollbar-hide pb-2">
            {[
              ...
              { id: 'status', label: '회차별 현황', icon: <Kanban size={16} strokeWidth={1.5} /> },
              ...
              {Object.keys(groupedData).map(phase => {
                ...
                return (
                  <div key={phase} id={`phase-${phase}`} className="rounded-2xl border p-6 transition-all" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-light)' }}>
                    <div 
                      className="flex justify-between items-center mb-5 pb-4 border-b cursor-pointer hover:opacity-70 transition-opacity" 
                      style={{ borderColor: 'var(--border-light)' }}
                      onClick={() => setSelectedProject(phaseItems[0])}
                    >
                      <div className="flex items-center gap-2">
                        <h4 className="font-bold text-[17px] tracking-tight" style={{ color: 'var(--text-main)' }}>{phase} 요약</h4>
                      </div>
                    ...
                    <div className="space-y-4">
                      {phaseItems.map((item, idx) => (
                        <div
                          key={idx}
                          className="flex justify-between items-center cursor-pointer p-2 rounded-xl transition-all hover-bg"
                          onClick={() => setSelectedProject(item)}
                        >
                          <div className="min-w-0 pr-4">
                            <div className="flex items-center">
                              <p className="font-semibold text-[14px] truncate" style={{ color: item['진행상황'] === '진행중' ? '#00D287' : (item['진행상황'] === '완료' ? (isDarkMode ? '#60A5FA' : '#3B82F6') : 'var(--text-muted)') }}>
                                {item['지구명']} {item['블록']}
                              </p>
                              {renderPMBadge(item)}
                            </div>
                            <p className="text-[11px] mt-1 truncate" style={{ color: 'var(--text-dim)' }}>{item['유형']}</p>
                          </div>
                          ...
                          return (
                            <div key={city} className="flex flex-col items-center justify-end h-full min-w-[44px] group relative cursor-pointer">
                              <div className="w-8 rounded-t-md transition-all duration-700 ease-out opacity-80 group-hover:opacity-100 relative flex justify-center" 
                                   style={{ height: `${heightPct}%`, background: isTop3 ? '#3B82F6' : 'var(--border)' }}>
                                <span className="absolute bottom-full mb-1.5 text-[12px] font-extrabold whitespace-nowrap flex items-baseline gap-0.5" style={{ color: isTop3 ? 'var(--text-main)' : 'var(--text-dimmer)' }}>
                      ...
                      <div className="flex-1 flex items-end justify-between w-full overflow-x-auto scrollbar-hide pb-2 pt-10 px-2 border-b border-dashed relative" style={{ borderColor: 'var(--border-light)' }}>
                        {sortedCitiesByBudget.map(([city, stats], index) => {
                          ...
                          return (
                            <div key={city} className="flex flex-col items-center justify-end h-full min-w-[44px] group relative cursor-pointer">
                              <div className="w-8 rounded-t-md transition-all duration-700 ease-out opacity-80 group-hover:opacity-100 relative flex justify-center" 
                                   style={{ height: `${heightPct}%`, background: isTop3 ? '#00D287' : 'var(--border)' }}>
                                <span className="absolute bottom-full mb-1.5 text-[12px] font-extrabold whitespace-nowrap flex items-baseline gap-0.5" style={{ color: isTop3 ? 'var(--text-main)' : 'var(--text-dimmer)' }}>
      ...
      <aside className="w-[260px] shrink-0 flex flex-col border-l overflow-y-auto scrollbar-hide transition-colors" style={{ background: 'var(--bg-side)', borderColor: 'var(--border)' }}>
        ...
        <div className="flex-1 px-4 pt-2 pb-5 space-y-6">
                    ...
                    <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 pr-8">
                      <div className="flex flex-col gap-4 shrink-0">
                        <div className="flex items-baseline gap-3 shrink-0">
                          <h3 className="text-[28px] font-bold tracking-tight whitespace-nowrap" style={{ color: 'var(--text-main)' }}>
                            [{selectedProject['회차']}]
                          </h3>
                          <p className="text-[17px] font-semibold break-keep" style={{ color: 'var(--text-main)' }}>
                            {allBlocksNames}
                          </p>
                        </div>

                        {/* ✅ 담당자 박스를 bg-main으로 눌러서 입체감 부여 */}
